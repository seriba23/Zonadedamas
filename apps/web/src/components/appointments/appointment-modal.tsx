            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                {appointment.client ? (
                  ...
                  {/* Alergias del cliente — alerta evidente (alergias en Client o en
                      la cuenta marketplace User). Solo si hay alergias registradas. */}
                  {(() => {
                    const allergies = (appointment.client as any)?.allergies || (appointment.client as any)?.user?.allergies;
                    if (!allergies) return null;
                    return (
                      <div className="mt-2 flex items-start gap-2 rounded-lg bg-red-50 border border-red-200 px-2.5 py-2">
                        <svg className="w-4 h-4 text-red-600 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                        </svg>
                        <p className="text-xs text-red-800 leading-snug">
                          <span className="font-semibold">Alergias del cliente:</span> {allergies}
                        </p>
                      </div>
                    );
                  })()}
