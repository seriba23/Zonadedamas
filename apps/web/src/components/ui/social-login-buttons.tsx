...
export function SocialLoginButtons({ onSocialLogin, disabled }: SocialLoginButtonsProps) {
  ...
  const handleGoogle = useCallback(() => {
    if (loading || disabled || !GOOGLE_CLIENT_ID) return;
    const google = (window as any).google;
    if (!google?.accounts?.id) return;

    google.accounts.id.prompt((notification: any) => {
      // If One Tap is dismissed or not available, use the popup flow
  ...
  const handleFacebook = useCallback(async () => {
    if (loading || disabled || !FACEBOOK_APP_ID) return;
    setLoading('facebook');
  ...
  const showFacebook = !!FACEBOOK_APP_ID;
    ...
    <div className="space-y-3">
      {showGoogle && (
        <button
          ...
          disabled={disabled || loading !== null || (!sdkReady.google && !!GOOGLE_CLIENT_ID)}
      ...
      {showFacebook && (
        <button
          ...
          disabled={disabled || loading !== null || (!sdkReady.facebook && !!FACEBOOK_APP_ID)}
