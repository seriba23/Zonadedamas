...
const navItems: NavItem[] = [
  ...
  // "Cupones" NO es sección nativa del empleado: solo aparece (como sección de
  // administración /rewards) si el dueño le concede el permiso rewards.create.
...
const REPORTS_ICON = I('M3 13.125C3 12.504 3.504 12 4.125 12h2.25c.621 0 1.125.504 1.125 1.125v6.75C7.5 20.496 6.996 21 6.375 21h-2.25A1.125 1.125 0 013 19.875v-6.75zM9.75 8.625c0-.621.504-1.125 1.125-1.125h2.25c.621 0 1.125.504 1.125 1.125v11.25c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V8.625zM16.5 4.125c0-.621.504-1.125 1.125-1.125h2.25C20.496 3 21 3.504 21 4.125v15.75c0 .621-.504 1.125-1.125 1.125h-2.25a1.125 1.125 0 01-1.125-1.125V4.125z');
// Icono de tarjeta para "Métodos de pago".
const PAYMENTS_ICON = I('M2.25 8.25h19.5M2.25 9h19.5m-16.5 5.25h6m-6 2.25h3m-3.75 3h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5z');
const CLIENTS_ICON = I('M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z');
...
export function EmployeeSidebar({ isOpen = false, onClose }: EmployeeSidebarProps = {}) {
  ...
  for (const item of navItems) {
    ...
    // Clientes: el freelancer también gestiona su cartera de clientes.
    if (item.href === '/employee/appointments' && isFreelancer) {
      menuItems.push({ label: 'Clientes', href: '/employee/clients', icon: CLIENTS_ICON });
    }
    // Reportes: solo el freelancer (métricas individuales de su propio negocio).
    if (item.href === '/employee/appointments' && isFreelancer) {
      menuItems.push({ label: 'Reportes', href: '/employee/reports', icon: REPORTS_ICON });
    }
  ...
  // ocultamos su versión NATIVA de empleado para no duplicar la entrada en el menú
  // (ambas abren lo mismo). La versión admin (más completa) la sustituye.
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3">
          <ul className="space-y-1">
            ...
            {/* Grupo "Reclutamiento": acceso al portal de reclutadores. Se
                muestra en NEGRO (no teal) para que sea distintivo, sin
                confundirse con las secciones normales del perfil. Al pulsarlo
                lleva al inicio de sesión de reclutadores (/creator/login); una
                vez dentro (si están aprobados) verán su portal. */}
            <li className="mt-3 pt-3 border-t border-gray-200">
              ...
              <Link
                href="/creator/intro"
                onClick={onClose}
                className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold bg-gray-900 text-white hover:bg-black transition-colors"
              >
                <NavIcon d={CREATOR_ICON} className="w-5 h-5 flex-shrink-0 text-white" />
