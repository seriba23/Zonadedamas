...
export default function EmployeeProfilePage() {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {employee && (
        <div className="mt-8">
          <h3 className="text-sm font-semibold text-gray-900 mb-3">Vista previa de tu perfil público</h3>
            ...
            <div className="rounded-2xl overflow-hidden relative" style={{ height: 200 }}>
              ...
              <div className="relative h-full flex flex-col justify-end p-4">
                <div className="flex items-end gap-3">
                  <div className="w-14 h-14 rounded-full border-2 border-white overflow-hidden flex-shrink-0 flex items-center justify-center" style={{ backgroundColor: (employee.color || '#008080') + '33' }}>
                    {employee.avatarUrl ? (
                      <img src={`${API_URL}${employee.avatarUrl}`} alt="" className="w-full h-full object-cover" />
                    ) : (
                      <span className="text-lg font-bold text-white">{employee.firstName[0]}{employee.lastName[0]}</span>
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold text-white leading-tight">{employee.firstName} {employee.lastName}</p>
                    {employee.jobTitle && (
                      <p className="text-[11px] text-white/80">{employee.jobTitle}</p>
                    )}
                    {employee.bio ? (
                      <p className="text-[10px] text-white/70 line-clamp-2 mt-0.5">{employee.bio}</p>
                    ) : (
                      <p className="text-[10px] text-white/50 italic mt-0.5">Sin descripción profesional</p>
                    )}
                  </div>
                </div>
                <div className="flex items-center gap-3 mt-3 pt-2 border-t border-white/20">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="text-xs font-semibold text-white">{stats?.averageRating?.toFixed(1) || '—'}</span>
                    <span className="text-[10px] text-white/60">({stats?.totalReviews || 0})</span>
                  </div>
                  <span className="text-[10px] text-white/60">{stats?.completedAllTime || 0} trabajos</span>
                  {employee.employeeServices && employee.employeeServices.length > 0 && (
                    <span className="text-[10px] text-white/60">{employee.employeeServices.length} servicio{employee.employeeServices.length !== 1 ? 's' : ''}</span>
                  )}
                </div>
              </div>
            </div>
          ...
          <p className="text-xs text-gray-400 mt-2">
            ...
            {!employee.bio && ' · Agrega una descripción en Configuración'}
          </p>
        </div>
      )}
