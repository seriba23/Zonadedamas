...
// Datos bancarios de Siliba para el pago por transferencia. Se pueden configurar
...
// Es un COMPONENTE propio (no una función render dentro del padre) para que su
// useState sea un hook normal. Antes era una función llamada condicionalmente
// (`modal === 'cancel-confirm' && renderCancelConfirm()`), lo que ejecutaba el
// hook de forma condicional → React error #310 al abrir el modal.
function CancelConfirmModal({ onClose, nextBillingDate, daysLeft, onConfirm, isPending }: {
  onClose: () => void;
    ...
    <Modal onClose={onClose}>
      ...
      <div className="px-6 py-5 space-y-4">
        ...
        <div className="rounded-xl px-4 py-3" style={{ backgroundColor: TEAL_LIGHT, border: `1px solid ${TEAL}` }}>
          ...
          <p className="text-base font-bold text-gray-900">{dayjs(nextBillingDate).format('D [de] MMMM [de] YYYY')}</p>
        ...
        <div className="flex gap-2">
          <button onClick={onClose} className="flex-1 py-2.5 rounded-xl text-sm font-medium border border-gray-200 text-gray-700 hover:bg-gray-50">Volver</button>
          <button onClick={onConfirm} disabled={typed.trim().toLowerCase() !== 'cancelar' || isPending}
      ...
      {sub && (
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          ...
          <div className="px-6 pb-5 flex gap-2 flex-wrap">
            ...
            {STRIPE_ENABLED && isActive && isMonthly && !sub.advancePaid && (
            ...
            {/* Activo: domiciliar tarjeta (guarda la tarjeta para cobro
                automático; requiere Stripe configurado). */}
            {STRIPE_ENABLED && isActive && (
