  ...
  return (
    <div className="p-4 md:p-6">
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <Link href="/inventory" className="shrink-0 inline-flex items-center gap-1.5 px-3 py-2.5 text-sm font-medium text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50">
      ...
      {detailProduct && (
        <ProductSalesModal product={detailProduct} onClose={() => setDetailProduct(null)} formatCurrency={formatCurrency} />
      )}
    </div>
  );
}

function ProductSalesModal({ product, onClose, formatCurrency }: { product: any; onClose: () => void; formatCurrency: (n: number) => string }) {
  const { data, isLoading } = useQuery({
  ...
  return (
    <Modal title={product.name} onClose={onClose} size="lg">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <Link href={`/inventory?focus=${product.id}`} className="shrink-0 text-xs font-medium text-[#008080] border border-teal-200 rounded-lg px-3 py-1.5 hover:bg-teal-50">
