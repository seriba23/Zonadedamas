  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-6">
        ...
        {/* Product Sales — usa el rango activo del selector de fechas */}
        <SalesCard
          startDate={bounds.start}
          endDate={bounds.end}
          periodLabel={rangeOptions.find((r) => r.key === dateRange)?.label}
...
function SalesCard({
  ...
  const { data: salesData } = useQuery({
    ...
    queryFn: () =>
      api.get<{ data: any }>(
        `/api/products/sales-stats?startDate=${startDate}&endDate=${endDate}`,
      ),
  });

  const sales = salesData?.data || { period: { count: 0, revenue: 0 }, recent: [] };
  ...
  return (
    <div className="bg-[var(--bg-surface)] rounded-xl border border-[var(--border)] p-3 md:p-5 mb-6">
      ...
      {/* Card unica con el revenue del periodo activo del selector de
          fechas global. Mismo estilo que el card "Venta Total" del header. */}
      <div className="rounded-xl p-3 md:p-5 overflow-hidden mb-5" style={{ backgroundColor: '#008080', color: 'white' }}>
        <div className="flex items-center gap-2 mb-1">
          <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'rgba(255,255,255,0.15)' }}>
        ...
        <p className="text-lg md:text-2xl font-extrabold leading-tight tabular-nums break-words" style={{ letterSpacing: '-0.015em' }}>
          {formatCurrency(sales.period.revenue)}
        </p>
        <p className="text-[10px] md:text-[11px] mt-1 opacity-80 truncate">
          {sales.period.count} venta{sales.period.count !== 1 ? 's' : ''}
