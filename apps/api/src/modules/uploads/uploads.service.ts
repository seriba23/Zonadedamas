...
export class UploadsService {
  constructor() {
    ...
    for (const folder of ['avatars', 'portfolio', 'documents', 'results']) {
  ...
  async saveFile(
    file: MulterFile,
    folder: 'avatars' | 'portfolio' | 'documents' | 'results',
  ): Promise<string> {
  ...
  async deleteFile(fileUrl: string): Promise<void> {
    ...
    const match = fileUrl.match(/\/api\/uploads\/(avatars|portfolio|documents|results)\/(.+)$/);
    if (!match) return;
