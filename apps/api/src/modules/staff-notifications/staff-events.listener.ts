  ...
  async handleAppointmentCreated(event: DomainEventPayload) {
    try {
      ...
      await this.notify.notify({
        ...
        title: 'Nueva cita',
        body: `${clientName} reservo ${services || 'una cita'} para el ${this.formatDateTime(apt.startTime)}.`,
        link: `/employee/appointments?focus=${apt.id}`,
        adminLink: `/calendar?focus=${apt.id}`,
  ...
  async handleAppointmentCancelled(event: DomainEventPayload) {
    try {
      ...
      await this.notify.notify({
        ...
        body: `${clientName} cancelo su cita del ${this.formatDateTime(apt.startTime)}.`,
        link: `/employee/appointments?focus=${apt.id}`,
        adminLink: `/calendar?focus=${apt.id}`,
        entityType: 'appointment',
        entityId: apt.id,
  ...
  async handleAppointmentRescheduled(event: DomainEventPayload) {
    try {
      ...
      await this.notify.notify({
        ...
        body: `${clientName} reagendo su cita al ${this.formatDateTime(apt.startTime)}.`,
        link: `/employee/appointments?focus=${apt.id}`,
        adminLink: `/calendar?focus=${apt.id}`,
        entityType: 'appointment',
        entityId: apt.id,
  ...
  async handleAppointmentConfirmed(event: DomainEventPayload) {
    try {
      ...
      await this.notify.notify({
        ...
        body: `${clientName} confirmo su cita del ${this.formatDateTime(apt.startTime)}.`,
        link: `/employee/appointments?focus=${apt.id}`,
        adminLink: `/calendar?focus=${apt.id}`,
        entityType: 'appointment',
        entityId: apt.id,
  ...
  async handlePurchaseCreated(payload: {
    ...
    try {
      ...
      await this.notify.notify({
        ...
        body: `${payload.purchase.customerName} compro ${itemsLabel} ($${payload.purchase.total.toFixed(2)}).`,
        link: `/dashboard/shop/orders/${payload.purchase.id}`,
        entityType: 'purchase',
  ...
  async handlePaymentCompleted(event: DomainEventPayload) {
    try {
      ...
      await this.notify.notify({
        ...
        section: 'payments',
        title: 'Pago recibido',
        body: `${clientName} pago ${Number(payment.totalAmount).toFixed(2)} ${payment.currency} (${payment.paymentMethod}).`,
        link: `/dashboard/payments?focus=${payment.id}`,
        entityType: 'payment',
        entityId: payment.id,
        audience: { kind: 'admins' },
      });
  ...
  async handleReviewCreated(payload: {
    ...
    try {
      await this.notify.notify({
        ...
        section: 'reviews',
        title: 'Nueva resena',
        body: `${payload.clientName} te dejo ${payload.rating} estrella${payload.rating === 1 ? '' : 's'}.`,
        link: `/employee/reviews?focus=${payload.reviewId}`,
        adminLink: `/reviews?focus=${payload.reviewId}`,
        entityType: 'review',
  ...
  async handleLowStock(payload: {
    ...
    try {
      await this.notify.notify({
        ...
        link: `/dashboard/inventory/products/${payload.productId}`,
        entityType: 'product',
  ...
  async handleReservationCreated(payload: {
    ...
    try {
      await this.notify.notify({
        ...
        body: `${payload.clientName} aparto ${payload.quantity}× ${payload.productName}.`,
        link: `/dashboard/inventory/reservations?focus=${payload.reservationId}`,
